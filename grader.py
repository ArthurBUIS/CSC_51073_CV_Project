...
def find_best_start_frame(X1, X2, window=10, start_v1=10):
    # indices into X
    landmark_ids=[k for k in range(33)]  # use all landmarks
    landmark_dim_idx = []
    ...
    for start in range(0, len(X2) - window - 1):
        ...
        # --- correction: detect inversion ---
        corr = np.mean(np.sum(win1_vel * win2_vel, axis=1))
        if corr < 0:
            win2_vel = -win2_vel  # automatic inversion
...
def detect_repetitions_ex(X, exercise, min_dist=40):
    """
    Repetition detection based on:
        • landmark given by exercise.landmark_id
        • axis (0=x,1=y,2=z)
        • opti: -1 -> minima, 1 -> maxima

    Args:
        X : array [frames, 99] = 33 landmarks * 3 coords
        exercise : ExerciseConfig
    """

    idx = exercise.landmark_id
    axis = exercise.axis        # 0, 1 or 2
    opti = exercise.opti        # -1 for minima, +1 for maxima

    # ---- 1) Extract the tracked signal ----
    coord = X[:, idx * 3 + axis]
    # ---- 2) Smoothing ----
    coord_smooth = savgol_filter(coord, window_length=21, polyorder=3)

    # ---- 3) Peak detection ----
    # opti = -1 -> minima -> invert the signal; opti = 1 -> maxima -> as-is
...
def compute_dtw_for_rep(X1, X2, start_v1, end_v1, start_v2):
    """
    Recompute DTW between:
      - segment V1 [start_v1 : end_v1]
      - V2 starting at start_v2

    Returns a dict: frame_v1 -> frame_v2
    """
    ...
    # IMPORTANT: the mapping must stay global in V1's timeline
...
def compute_repgrade(df, name_exercise):
    ...
    rep_starts = detect_repetitions_ex(X1, EXERCISES[name_exercise])  # array of rep start frames
    ...
    # --- Last repetition (otherwise it's missed!) ---
